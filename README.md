...
All panels now share a common title/header/footer renderer and a unified set
of highlight groups. You can override any group through
`setup({ highlights = { ... } })`.
...
require("gitflow").setup({
  ...
  icons = {
    enable = false,             -- Nerd Font icons; false = ASCII fallback
  },
})
```
...
| `icons.enable` | `boolean` | `false` | Use Nerd Font icons; `false` = ASCII |
