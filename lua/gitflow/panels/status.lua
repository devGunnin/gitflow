...
local function append_file_section(title, entries, spec, line_entries, diff_staged)
	...
	for _, entry in ipairs(entries) do
		...
		local text = ("%s %s  %s"):format(icon, status, entry.path)
		local line = ui_render.entry(spec, text, group)
		line_entries[line] = {
			kind = "file",
			entry = entry,
...
local function append_commit_section(title, entries, spec, line_entries, pushable)
	ui_render.section(spec, title)
	if #entries == 0 then
		ui_render.empty(spec, "none")
		ui_render.blank(spec)
	...
	for _, entry in ipairs(entries) do
		...
		local icon = icons.get("git_state", "commit")
		local line = ui_render.entry(spec, ("%s %s"):format(icon, entry.summary))
		line_entries[line] = {
			kind = "commit",
			entry = entry,
...
local function render(grouped, outgoing_entries, incoming_entries, upstream_name, current_branch)
	...
	if upstream_name then
		...
		append_commit_section(outgoing_title, outgoing_entries, spec, line_entries, true)

		local incoming_title = ("Incoming (oldest -> newest, only on %s)"):format(upstream_name)
		append_commit_section(incoming_title, incoming_entries, spec, line_entries, false)
	else
		ui_render.section(spec, "Outgoing / Incoming")
		ui_render.empty(spec, "upstream branch is not configured")
		ui_render.blank(spec)
	end
