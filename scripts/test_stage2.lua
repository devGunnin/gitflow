...
assert_true(
	find_line(status_lines, expected_branch_line) ~= nil,
	"status panel should display current branch"
)
assert_true(
	find_line(status_lines, "[q] Close") ~= nil,
	"status panel should render themed footer key hints"
)
...
local diff_ready = vim.wait(5000, function()
	...
	return find_line(lines, expected_branch_line) ~= nil and find_line(lines, "[q] Close") ~= nil
end, 25)
assert_true(diff_ready, "diff panel should render branch line and themed footer")
...
assert_true(log_ready, "log panel should render branch line and themed footer")
...
local stash_panel_ready = vim.wait(5000, function()
	...
	return find_line(lines, "Gitflow Stash") ~= nil
		and find_line(lines, expected_branch_line) ~= nil
		and find_line(lines, "[q] Close") ~= nil
end, 25)
...
assert_true(
	find_line(stash_lines, expected_branch_line) ~= nil,
	"stash panel should display current branch"
)
