...
create_issue_if_missing() {
  ...
  while IFS=$'\t' read -r number state url existing_title; do
    if [ -z "${number:-}" ]; then
      continue
    fi

    if [ "$existing_title" = "$title" ]; then
      echo "Skipping existing issue #${number} (${state}): ${title}"
      skipped_count=$((skipped_count + 1))
      return 0
    fi
  done < <(find_existing_issues "$title")
...
create_issue_if_missing "[Stage 1] Bootstrap plugin architecture and async command runtime" <<'EOF_STAGE_1'
Goal
...
create_issue_if_missing "[Stage 2] Implement core git workflows in Neovim UI" <<'EOF_STAGE_2'
Goal
...
create_issue_if_missing "[Stage 3] Add GitHub integration layer and single-stroke sync" <<'EOF_STAGE_3'
Goal
...
create_issue_if_missing "[Stage 4] Build issue management UI (create, edit, label, comment)" <<'EOF_STAGE_4'
Goal
...
create_issue_if_missing "[Stage 5] Build PR dashboard and review interactions" <<'EOF_STAGE_5'
Goal
...
create_issue_if_missing "[Stage 6] Implement merge conflict center and resolution helpers" <<'EOF_STAGE_6'
Goal
...
create_issue_if_missing "[Stage 7] Add Neovim-native theming, highlights, and UX polish" <<'EOF_STAGE_7'
Goal
...
create_issue_if_missing "[Stage 8] Finalize docs, keybind defaults, tests, and release readiness" <<'EOF_STAGE_8'
Goal
...
echo "Issue creation complete for ${REPO}: created=${created_count}, skipped_existing=${skipped_count}."
